...
def print_epoch_loss(phase, running_loss, running_corrects, optimizer, dataset, since):
    with torch.no_grad():
...
def train_model(model, train_dataset, val_dataset, optimizer, criterion, batch_size=16, num_epochs=5):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in train_dataloader:
            ...
            targets = targets.unsqueeze(dim=1).float().to(device)
        ...
        print_epoch_loss('train', train_loss, train_corrects, optimizer, train_dataset, since)
        ...
        with torch.no_grad():
            ...
            print_epoch_loss('validation', val_loss, val_corrects, optimizer, val_dataset, since)
    ...
    torch.save(model, '/home/tstrebel/models/rnsa-densenet.pt')
