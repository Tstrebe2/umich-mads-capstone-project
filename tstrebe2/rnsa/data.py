...
class RNSAIndexedDataset(RNSADataset):
    def __init__(self, 
                 img_dir:str, 
                 img_targets:pd.DataFrame, 
                 transform:torchvision.transforms.Compose=None, 
...
def get_dataset(img_dir:str, df:pd.DataFrame, train:bool=False, indexed:bool=False) -> Dataset:
    '''
    img_dir: directory where rnsa dataset image files are stored.
    ...
    if indexed:
        return RNSAIndexedDataset(img_dir, df, transform, target_transform)
    else:
